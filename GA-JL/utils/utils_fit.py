...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        #   保存权值
        #-----------------------------------------------#
        #if ((epoch + 1) %  save_period == 0 and epoch > 85)  :
        if ((epoch + 1) % (save_period) == 0 and epoch > 0) or epoch + 1 == Epoch :
            #if ((epoch + 1) % (save_period/2) == 0  and epoch>25) :
        ...
        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, "epbest.pth"))
