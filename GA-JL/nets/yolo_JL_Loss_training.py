...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, input_shape, cuda, anchors_mask = [[6,7,8], [3,4,5], [0,1,2]], label_smoothing = 0, focal_loss = False, alpha = 0.25, gamma = 2, iou_type = 'ciou_NWD_1'):
    ...
    def box_iou(self, b1, b2):
        ...
        #----------------------------------------------------#
        #print(b1)
        #print(b2.shape)  #torch.Size([20, 3, 80, 64, 4])
        ...
        wh_distance = ((b1[..., 2] - b2[..., 2]) ** 2 + (b1[..., 3] - b2[..., 3]) ** 2) / 4
        wasserstein_2 = center_distance + wh_distance
        wasserstein_2 = torch.exp(-torch.sqrt(wasserstein_2) / constant)
        ...
        if self.iou_type == 'ciou':
            ...
            out     = iou - center_distance

        elif self.iou_type == 'ciou_NWD_1':
            # ----------------------------------------------------#
            #   计算中心的距离
            # ----------------------------------------------------#
            center_distance = torch.sum(torch.pow(center_wh, 2), axis=-1)

            ciou = iou - center_distance
            # print("ciou:{}".format(ciou))

            out = 2 * ciou + wasserstein_2
    ...
    def forward(self, l, input, targets=None):
        ...
        if n != 0:
            ...
            loss_loc    = torch.mean(0.5 * (3 - iou)[obj_mask])
