...
class CoorAttn(nn.Module):
    ...
    def forward(self,x):
        ...
        #print("x_h.type: " + type(x_h))
        #print("x_h:{}".format(x_h.shape))
        #print("x_w:{}".format(x_w.shape))
...
class YoloBody(nn.Module):
    ...
    def forward(self,x):
        ...
        # PFF
        PFF1_1 = self.PFF1_conv(x1)  # 16,16,512
        PFF1_2 = self.PFF1_upsample_2(PFF1_1)  # 32,32,256

        PFF3_3 = self.PFF3_conv(x3)  # 64,64,128
        PFF3_2 = self.PFF3_downsample_2(PFF3_3)  # 32,32,256
        ''''## 实验的时候，好像写错了，加载pth文件的时候记得改, 应该改成下面这个状态
        PFF1_2 = self.PFF2_conv(x2)  # 32,32,256        

        #x = torch.cat([x, x2, x3, x4], dim=1)  # ds,4C,H,W
        x2_res=torch.cat([PFF1_2, PFF1_2, PFF3_2], dim=1)'''

        PFF2_2 = self.PFF2_conv(x2)  # 32,32,256
        x2_res = torch.cat([PFF1_2, PFF2_2, PFF3_2], dim=1)

        x2_res = self.PFF2_cat_conv(x2_res)

        x2_res = self.coorAttn(x2_res)
        x2 = x2 + x2_res
